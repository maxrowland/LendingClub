...
#Script uses a PyTorch Neural Net classifier to predict loan defaults within the lending Club dataset
#%%General Packages
import os, errno, time, smtplib, ssl
...
#######################################################################################################################
#######################################################################################################################
model_name = 'Neural Network' #Name used in chart titles
save_path = r'C:\Users\mhr19\Dropbox\CODE\CONSUMER_DEBT\CL_NN' #Directory for saving model output
...
y_val_t = torch.from_numpy(y_val.values).float()
...
n_batch = 32
n_epochs = 10
learning_rate = 1e-3 #Maximum learning rate for parameters, minimum = 0
n_input_dim = X_train.shape[1]
weight_decay = 0 #Higher = slows learning rate

torch.manual_seed(1989)
class_sample_count = np.array([len(np.where(y_train['charged_off']==t)[0]) for t in np.unique(y_train)])
weight = 1. / class_sample_count
samples_weight = np.array([weight[t] for t in y_train['charged_off']])
samples_weight = torch.from_numpy(samples_weight)
sampler = WeightedRandomSampler(weights=samples_weight.type('torch.DoubleTensor'), num_samples=len(samples_weight), replacement=True)

train_loader = DataLoader(dataset=train, batch_size=n_batch, pin_memory=True, sampler=sampler)
val_loader = DataLoader(dataset=val, batch_size=n_batch, shuffle=True, pin_memory=True)
test_loader = DataLoader(dataset=test, batch_size=n_batch, shuffle=True, pin_memory=True)

class net(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(n_input_dim, 500),
            nn.Sigmoid(),
            nn.Linear(500, 250),
...
for epoch in range(n_epochs):
    ...
    with torch.no_grad():
        val_losses = []
        for x_val_nn, y_val_nn in test_loader:
            x_val_nn = x_val_nn.to(device)
            y_val_nn = y_val_nn.to(device)
            model.eval()
            yhat = model(x_val_nn)
            val_loss = loss_fn(yhat, y_val_nn).item()
            val_losses.append(val_loss)
        validation_loss = np.mean(val_losses)
        validation_losses.append(validation_loss)

    #torch.save(model.state_dict(), os.path.join(folder_path, 'epoch-{}.pt'.format(epoch+1)))
    torch.save(model, os.path.join(folder_path, 'epoch-{}.pt'.format(epoch + 1)))
...
best_model_params = torch.load(os.path.join(folder_path, 'epoch-{}.pt'.format(best_epoch)))

#print model
print(model)
#plot training loss over epochs
plt.plot(training_losses, color="#008AC9", label='Train')
plt.plot(validation_losses, color="#f11a21", label='Validation')
plt.ylabel('Loss')
plt.xlabel('Epoch')
plt.title('Training & Validation Loss')
plt.grid(linestyle='--')
plt.legend()
plt.savefig(folder_path + "\\" + 'Training Results', dpi=200)
plt.tight_layout()
plt.show()
#######################################################################################################################
#######################################################################################################################
#######################################################################################################################
#%%Model Results
def model_results(model_name, X, y, val_or_test, threshold, window):
    ...
    # Load Index Returns
    mkt_ret = pd.read_excel(r'E:\DATABASES\LENDING_CLUB\MODEL DATA\ECON_DATA.XLSX', sheet_name='RETURNS',
    ...
    if val_or_test is 'val':
        ...
        X_t = X_test_t
        index = y_test.index
        val_or_test_label = 'TEST'

    best_model_params.eval().cpu()
    y_val_score = best_model_params(X_t).cpu().detach()
    y_val_score = y_val_score.cpu().detach().numpy()
    y_val_pred = np.where(y_val_score > threshold, 1, 0)
    ...
    val_loans_charge_off['MONTH'] = val_loans_charge_off['MONTH'].astype(
        'datetime64[M]') - pd.tseries.offsets.MonthEnd(0)
    ...
    val_date_range = 'Returns Date Range: ' + bm_val_start.strftime('%m/%d/%Y') + ' to ' + bm_val_end.strftime(
        '%m/%d/%Y')  # Table Date Span Text
    val_ret_index = pd.date_range(start=bm_val_start, end=bm_val_end,
                                  freq='M')  # create datetime index from start to end dates

    # Create return dataframe
    val_ret = pd.DataFrame(index=val_ret_index)
    val_ret = pd.merge(val_ret, mkt_ret, left_index=True,
                       right_index=True)  # merge public market returns to ret dataframe
    val_ret = val_ret.merge(bm_val, how='left', left_index=True, right_index=True).fillna(
        0)  # merge equal weight benchmark to ret dataframe
    ...
    y_val_unstack = pd.pivot_table(y, values='charged_off', columns='id')
    ...
    balanced_accuracy_val = metrics.balanced_accuracy_score(y, y_val_pred,
                                                            adjusted=False)  # adjusted: 1=best 0=worst
    ...
    print('----------' + val_or_test_label + ' SUMMARY RESULTS----------',
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    ...
    print('Balanced Accuracy: '"{:0.4}".format(balanced_accuracy_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    ...
    print('Average Precision: '"{:0.4}".format(average_precision_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Precision: ', precision_val, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Precision Micro: '"{:0.4}".format(precision_micro_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Precision Macro: '"{:0.4}".format(precision_macro_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Precision Weighted: '"{:0.4}".format(precision_weighted_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    ...
    print('Recall Micro: '"{:0.4}".format(recall_micro_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Recall Macro: '"{:0.4}".format(recall_macro_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Recall Weighted: '"{:0.4}".format(recall_weighted_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    ...
    print('Jaccard Micro: '"{:0.4}".format(jaccard_micro_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Jaccard Macro: '"{:0.4}".format(jaccard_macro_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Jaccard Weighted: '"{:0.4}".format(jaccard_weighted_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('---------ROC METRICS---------', file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('ROC = Area Under the Receiver Operating Characteristic Curve',
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('ROC AUC: '"{:0.4}".format(roc_auc_val), file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('ROC AUC Micro: '"{:0.4}".format(roc_auc_micro_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('ROC AUC Macro: '"{:0.4}".format(roc_auc_macro_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('ROC AUC Weighted: '"{:0.4}".format(roc_auc_weighted_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('ROC AUC Samples: '"{:0.4}".format(roc_auc_samples_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('---------OTHER METRICS---------', file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Brier Score (lower=better): '"{:0.4}".format(brier_score_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Cross-entropy Loss (lower=better): '"{:0.4}".format(neg_log_loss_val),
          file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    ...
    print('Model: ', model, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Best Epoch: ', best_epoch, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Batches: ', n_batch, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Epochs: ', n_epochs, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Optimizer: ', optimizer, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Learning Rate: ', learning_rate, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Decay Weight: ', weight_decay, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    # %%Return Calculations
    # Performance
    ...
    # Alpha
    alpha_rolling = ret_rolling - beta_rolling.mul((ret_rolling[benchmark].sub(ret_rolling[cash], axis=0)),
                                                   axis=0).add(ret_rolling[cash], axis=0)
    ...
    bm_val_int_rate = bm_val_int_rate.mul(bm_val_weights, axis='index').sum(axis=1).rename('bm_int_rate')
    ...
    port_val_int_rate = port_val_int_rate.mul(port_val_weights, axis='index').sum(axis=1).rename('port_int_rate')
    ...
    bm_val_fico = bm_val_fico.mul(bm_val_weights, axis='index').sum(axis=1).rename('bm_fico')
    ...
    port_val_fico = port_val_fico.mul(port_val_weights, axis='index').sum(axis=1).rename('port_fico')
    ...
    bm_val_loan_amnt = bm_val_loan_amnt.mul(bm_val_weights, axis='index').sum(axis=1).rename('bm_loan_amnt')

    port_val_loan_amnt = pd.DataFrame(index=val_ret_index, columns=port_val_loans_ret.columns)
    port_val_loan_amnt = port_val_loan_amnt.where(port_val_loans_ret.isnull(), port_val_loans.loc['loan_amnt'],
                                                  axis=1)
    port_val_loan_amnt = port_val_loan_amnt.mul(port_val_weights, axis='index').sum(axis=1).rename('port_loan_amnt')
    ...
    bm_val_dti = bm_val_dti.mul(bm_val_weights, axis='index').sum(axis=1).rename('bm_dti')
    ...
    port_val_dti = port_val_dti.mul(port_val_weights, axis='index').sum(axis=1).rename('port_dti')
    ...
    bm_val_term_avg = bm_val_term.mul(bm_val_weights, axis='index').sum(axis=1).rename('bm_term')
    ...
    port_val_term_avg = port_val_term.mul(port_val_weights, axis='index').sum(axis=1).rename('port_term')
    ...
    bm_val_credit_avg = bm_val_credit.mul(bm_val_weights, axis='index').sum(axis=1).rename('bm_grade')
    ...
    port_val_credit_avg = port_val_credit.mul(port_val_weights, axis='index').sum(axis=1).rename('port_grade')
    ...
    val_charge_off_rolling = val_loans_charge_off.eq(1).sum(axis=1).rename(
        'val_charge_off_rolling') / bm_val_n_loans
    ...
    print('BM A: ', bm_val_a_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    bm_val_b_default = bm_val_b_co / bm_val_b
    print('BM B: ', bm_val_b_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    bm_val_c_default = bm_val_c_co / bm_val_c
    print('BM C: ', bm_val_c_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    bm_val_d_default = bm_val_d_co / bm_val_d
    print('BM D: ', bm_val_d_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    bm_val_e_default = bm_val_e_co / bm_val_e
    print('BM E: ', bm_val_e_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    bm_val_f_default = bm_val_f_co / bm_val_f
    print('BM F: ', bm_val_f_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    bm_val_g_default = bm_val_g_co / bm_val_g
    print('BM G: ', bm_val_g_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    ...
    print('Port A: ', port_val_a_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    port_val_b_default = port_val_b_co / port_val_b
    print('Port B: ', port_val_b_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    port_val_c_default = port_val_c_co / port_val_c
    print('Port C: ', port_val_c_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    port_val_d_default = port_val_d_co / port_val_d
    print('Port D: ', port_val_d_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    port_val_e_default = port_val_e_co / port_val_e
    print('Port E: ', port_val_e_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    port_val_f_default = port_val_f_co / port_val_f
    print('Port F: ', port_val_f_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))
    port_val_g_default = port_val_g_co / port_val_g
    print('Port G: ', port_val_g_default, file=open(folder_path + "\\" + "DEFAULT_RATES.txt", "a"))


    char_export = pd.DataFrame(index=val_ret_index)
    char_export = pd.merge(bm_val_int_rate, port_val_int_rate, left_index=True,right_index=True)
    char_export = char_export.merge(bm_val_fico, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(port_val_fico, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(bm_val_loan_amnt, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(port_val_loan_amnt, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(bm_val_dti, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(port_val_dti, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(bm_val_term_avg, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(port_val_term_avg, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(bm_val_credit_avg, how='left', left_index=True, right_index=True).fillna(0)
    char_export = char_export.merge(port_val_credit_avg, how='left', left_index=True, right_index=True).fillna(0)
    ...
    with pd.ExcelWriter(folder_path + "\\" + "RESULTS.xlsx") as writer:
        ...
        val_ret.to_excel(writer, sheet_name=val_or_test_label + "_RETURNS", index=True)
        char_export.to_excel(writer, sheet_name=val_or_test_label + "_CHAR", index=True)
    ...
    ax[1, 1].text(std_incep[benchmark] + .0025, ret_incep[benchmark] + .0025, 'Benchmark',
                  horizontalalignment='left',
                  verticalalignment='top')
    ax[1, 1].scatter(std_incep[portfolio], ret_incep[portfolio], label='Portfolio', color=mblue)
    ax[1, 1].text(std_incep[portfolio] + .0025, ret_incep[portfolio] + .0025, 'Portfolio',
                  horizontalalignment='left',
                  verticalalignment='top')
    ...
    # Correlation Matrix
    labels = corr_incep.rename(columns={'Benchmark': 'bm_val', 'Portfolio': 'port_val'},
                               index={'Benchmark': 'bm_val', 'Portfolio': 'port_val'})
    ...
    print('Optimal ROC Threshold: ', optimal_thresh_roc, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
    print('Optimal PR Threshold: ', optimal_thresh_pr, file=open(folder_path + "\\" + "MODEL_OUTPUT.txt", "a"))
